  ...
  function updateMarkers(m) {
    ...
    bares.forEach(bar => {
      ...
      mk.bindPopup(`
        <b>${bar.name}</b><br/>
        ${bar.barrio || ''}<br/>
        ${bar.precio_cana ? '🍺 ' + bar.precio_cana + '<br/>' : ''}
        ★ ${bar.avgScore ? bar.avgScore.toFixed(1) : '—'}<br/>
        ${visited ? '✓ Estuviste aquí' : `<button id="ci-${bar.id}" style="margin-top:6px;padding:4px 10px;background:#6B5EA8;color:white;border:none;border-radius:6px;cursor:pointer;">Estuve aquí</button>`}
      `)
      mk.on('popupopen', () => {
